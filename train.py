...
# from exper_load_pytorch_weight import load_pytorch_weight

def update_average(model_target, model_source, beta=0.999):
    p_src = model_source.get_weights()
    p_tgt = model_target.get_weights()
    assert(len(p_src) == len(p_tgt))
    for i in range(len(p_src)):
        p_tgt[i] = beta*p_tgt[i] + (1. - beta)*p_src[i]
    model_target.set_weights(p_tgt)
...
def save_result(network, dataset, key_str, save_root, display_max, isTrain = False):
    ...
    filename = "EMA_train" if isTrain else "EMA_test"
    filename = filename + "_%s_%02d"
    for idx, (co_data, cl_data) in dataset.enumerate():
        return_dict = network.test_EMA_step(co_data, cl_data)
        display_list = []
        display_imgs = []
        for key in return_dict:
            display_list.append(key)
...
if __name__ == "__main__":
    ...
    gen_test_ckpt_prefix = os.path.join(checkpoint_dir, "gen_test_ckpt")
    ...
    with strategy.scope():
        ...
        gen_test_ckpt = tf.train.Checkpoint(net= network.gen_test)
    ...
    def train_ds_fn(input_context):
        ...
        return d.shard(input_context.num_input_pipelines, input_context.input_pipeline_id).repeat()
    train_dist_dataset = strategy.distribute_datasets_from_function(train_ds_fn)
    ...
    for num_iter in range(opts.start_iter,MAX_ITER+1):
        ...
        update_average(network.gen_test, network.gen)
        ...
        if num_iter % CONFIG['snapshot_save_iter'] == 0:
            ...
            gen_test_path = gen_test_ckpt_manager.save()
            print("save generator checkpoint: ", gen_path)
            print("save generator-EMA checkpoint: ", gen_test_path)
